...
class Variable(Term):
    ...
    def __init__(self, prefix: VarPrefix, word: str, idx: int = 0, do_hashing=False) -> None:
        ...
        # only for dependent variable. TODO: implement son classes of Variable, including DependentVar, IndependentVar, QueryVar.
        self.dependents = []
        ...
        # if idx is not None:
        if self.is_ivar:
            self._vars_independent.add(IntVar(int(idx)), [])
        if self.is_dvar:
            self._vars_dependent.add(IntVar(int(idx)), [])
        if self.is_qvar:
            self._vars_query.add(IntVar(int(idx)), [])
